...
class SentencePredictionCriterionCRD(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.num_samples > 0:
            ...
            # hidden_state_teacher = torch.cat([t.transpose(0, 1).mean(1) for t in extra_teacher['inner_sstates'][1:]], dim=1)
           
           
            # hidden_stat = torch.cat([t.transpose(0, 1)[:, 0] for t in extra['inner_states'][1:]], dim=1)
            # # # 暂时把student也隔两层一抽
            # # hidden_stat = torch.cat([t.transpose(0, 1)[:, 0] for t in extra['inner_states'][1::2]], dim=1)
            # # teacher hidden_state每隔两层提取一个
            hidden_state_teacher = torch.cat([t.transpose(0, 1)[:, 0] for t in extra_teacher['inner_states'][4::4]], dim=1)
