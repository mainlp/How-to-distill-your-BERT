...
class BasePretrainModel(object):
    ...
    def forward(self, batch, output_attentions=False, output_qkv=False, output_loss=True, output_hidden_states=False):
        outputs = self.network(batch, output_attentions, output_qkv, output_loss, output_hidden_states)
...
class BasePretrainModelHuggingFace(object):
    def __init__(
        ...
        if not model_type:
            # getting default model type from args
            model_type = args.model_type
        assert model_type in HFMODELS, f"model_type {model_type} is not supported"
        model_cls, config_cls, token_cls = HFMODELS[model_type]
